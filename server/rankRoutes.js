...
async function upsertKeywordGroup({ project, keyword, domains, targetUrl }) {
  ...
  const group = (await pool.query(`INSERT INTO rank_keyword_groups (project_name, keyword, keyword_lc) VALUES ($1,$2,$3) ON CONFLICT (keyword_lc) DO UPDATE SET project_name=COALESCE(NULLIF(EXCLUDED.project_name,''), rank_keyword_groups.project_name), keyword=EXCLUDED.keyword RETURNING *`, [project || "", keywordClean, keywordLc])).rows[0];
