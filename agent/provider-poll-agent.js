...
async function checkDomain(domain) {
  ...
  try {
    const response = await axios.get(`https://${cleanDomain}`, {
      ...
      maxRedirects: 5,
      validateStatus: () => true,
      headers: { "User-Agent": `DomainRadarPollAgent/1.0 ${PROVIDER_NAME}` }
    });
...
async function pollOnce() {
  const { data } = await axios.post(`${CENTRAL_URL}/api/agent/poll`, {
    node_name: NODE_NAME,
  ...
  await axios.post(`${CENTRAL_URL}/api/agent/result`, {
    node_name: NODE_NAME,
    secret_key: AGENT_SECRET,
    task_id: id,
    result
...
async function loop() {
  ...
  while (true) {
    try {
      ...
      console.error(`[${new Date().toISOString()}] Poll error: ${err.response?.data?.error || err.code || err.message}`);
    }
